...
function empiricalDNAfrequencies(
    ...
    for j in axes(dnaDat, 2) # for each column
        col = dnaDat[!,j]
        wt = dnaWeights[j]
...
stationary(::JC69) = [0.25,0.25,0.25,0.25]
stationary(mod::HKY85) = mod.pi
stationary(mod::ERSM) = [1/mod.k for i in 1:mod.k]
stationary(mod::BTSM) = [mod.eigeninfo[2], mod.eigeninfo[3]]
