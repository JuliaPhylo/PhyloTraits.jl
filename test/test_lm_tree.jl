...
library(geiger)
data("caudata") # (salamanders)
write.tree(caudata$phy, file="caudata_tree.txt", append=F, digits=10, tree.names=F)
V <- node.depth.edgelength(caudata$phy) # time shared
prac <- mrca(caudata$phy, full = TRUE)
V <- matrix(V[prac], dim(prac))
write.table(V, file="caudata_shared_paths.txt", sep = ",", row.names=F, col.names=T)
=#
...
data("caudata") # salamanders
write.tree(caudata$phy, file="caudata_tree.txt", append=F, digits=10, tree.names=F)
write.table(data.frame(tipsNames = names(caudata$dat), trait = unname(caudata$dat)),
            file = "caudata_trait.txt", sep = ",", row.names = FALSE)
# fit using Geiger
fitgeiger <- fitContinuous(caudata$phy, caudata$dat, model = "BM")
# fit using phylolm
library(phylolm)
fitphylolm <- phylolm(trait ~ 1, data.frame(trait = caudata$dat), caudata$phy, model="BM")
# fit using Rphylopars
library(Rphylopars)
fitphylopars <- phylopars(data.frame(species = names(caudata$dat),
    trait = unname(caudata$dat)), caudata$phy, pheno_error=F, pheno_correlated=F,
    REML = FALSE)
...
fitphylolm <- phylolm(trait ~ shift_37 + shift_105 + shift_222, 
    data.frame(trait=dat, shift_37 = Tm[,37], shift_105 = Tm[,105], shift_222 = Tm[,222]),
