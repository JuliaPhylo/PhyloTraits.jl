...
dfr_shift = descendencedataframe(net, net.edge[[8,17]])
dfr_shift[!,:sum] = vec(sum(Matrix(dfr_shift[:,findall(DataFrames.propertynames(dfr_shift) .!= :tipnames)]), dims=2))
dfr_hybrid = descendencedataframe(net)
@test_throws ArgumentError descendencedataframe(net, :something) # wrong 'which'
...
@test lambda_estim(phynetlm) ≈ 0.8356905283 atol=1e-7
